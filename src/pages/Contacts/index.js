        ...
        <Page className='contacts' loading={loading}>
            <div className='page-header'>
                ...
                <div className='buttons'>
                    <Button className={`${!viewMode ? 'active' : ''}`} onClick={() => setViewMode(false)} shape="circle"
                            icon={<AppstoreOutlined/>}/>
                    <Button className={`${viewMode ? 'active' : ''}`} onClick={() => setViewMode(true)} shape="circle"
                            icon={<BarsOutlined/>}/>
                </div>
            </div>
            <div className='main-page'>
                <FilterBar onFilter={onFilter}/>
                {viewMode ? <Tabular list={data} pagination={pagination} onPagination={onPagination}/> :
                    <Tiled list={data}/>}
            </div>
        </Page>
    )
